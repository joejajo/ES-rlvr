#!/usr/bin/env python3
"""
ES-RLVR (Pure Binary): Evolution Strategies with vLLM + Ray + NCCL
====================================================================
Architecture: adapted from VsonicV/es-fine-tuning-paper
Methodology:  pure ES with binary reward — NO entropy bonus

This script is a stripped variant of es_rlvr_train.py with ALL entropy
terms removed.  The reward is simply:

    total = binary_reward    (1.0 correct, 0.0 wrong)
...
style ES from es-fine-tuning-paper exactly.
...
NUM_ITERATIONS   = 200
VAL_EVERY        = 10
EXPERIMENT_DIR   = "outputs/es_pure_binary"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ES (pure binary reward): vLLM+Ray+NCCL, no entropy bonus"
    )
...
def save_plots(history: dict, output_dir: str):
    ...
    fig.suptitle(
        "ES (Pure Binary)  ·  Single Example Training → Math500 Generalisation",
    ...
    ax.set_xlabel("Iteration")
    ax.set_ylabel("Binary Reward")
    ax.set_title("ES Population Reward (binary)")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
...
def main(args):
    ...
    print("=" * 70)
    print("  ES (Pure Binary): vLLM + Ray + NCCL  [NO entropy bonus]")
    print("=" * 70)
    ...
    print(f"  Reward         : binary only (no entropy bonus)")
    ...
    for i in range(args.num_iterations):
        ...
        if args.antithetic:
            ...
            seed_tasks = []
            for s in base_seeds:
                seed_tasks.append((s, False))
                seed_tasks.append((s, True))
        else:
