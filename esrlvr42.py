#!/usr/bin/env python3
"""
ES-RLVR (seed=42): Evolution Strategies + One-Shot-RLVR with vLLM + Ray + NCCL
================================================================================
...
Key differences from es_rlvr_train.py
--------------------------------------
- SamplingParams seed=42  (fixed sampling seed — cleaner ES gradient signal)
- global_seed default=None (perturbation seeds remain stochastic across runs)
- max_tokens=3048
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Default hyperparameters  (matches es_fine_tuning_deepscaler_accl.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ES-RLVR seed=42: One-Shot-RLVR in a vLLM+Ray+NCCL ES loop (fixed sampling seed)"
    )
...
def main(args):
    ...
    print("=" * 70)
    print("  ES-RLVR (sampling seed=42) — One-Shot-RLVR + vLLM + Ray + NCCL")
    print("=" * 70)
    ...
    for i in range(args.num_iterations):
        ...
        # GRPO / ES reward normalisation: Aᵢ = (rᵢ − mean) / (std + ε)
        ...
        # ES weight update: Δθ += (α/N) × Aᵢ × εᵢ  on engine 0
