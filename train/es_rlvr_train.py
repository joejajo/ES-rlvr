...
def parse_args():
    ...
    parser.add_argument("--val_parquet_path", type=str,
                        ...
                        help="Validation parquet for inline eval (math500).")
    # ES
    ...
    parser.add_argument("--val_every", type=int, default=0,
                        help="Run inline math500 validation every N iterations (0 = disabled).")
...
def evaluate_handle(llm, task_datas: list, temperature: float = 0.7,
    ...
    sampling_params = SamplingParams(
        ...
        seed=42,             # fixed sampling seed, matches countdown_accl reference
    )
...
def main(args):
    ...
    logging_dir = os.path.join(args.experiment_dir, f"run_{run_tag}")
    tb_dir      = os.path.join(args.experiment_dir, "tb", f"run_{run_tag}")
    writer = SummaryWriter(log_dir=tb_dir)
    model_saves_dir = os.path.join(logging_dir, "model_saves")
    os.makedirs(model_saves_dir, exist_ok=True)
    os.makedirs(os.path.join(args.experiment_dir, "logs"),       exist_ok=True)
    os.makedirs(os.path.join(args.experiment_dir, "train_preds"), exist_ok=True)
    os.makedirs(os.path.join(args.experiment_dir, "val_preds"),   exist_ok=True)
    ...
    val_task_datas = []
    if args.val_every > 0 and args.val_parquet_path:
        if os.path.exists(args.val_parquet_path):
            ...
            print(f"[DATA] val_parquet_path not found, inline val disabled: {args.val_parquet_path}")
            args.val_every = 0
    ...
    train_outputs_jsonl = os.path.join(
        args.experiment_dir, "train_preds", f"train_outputs_{run_tag}.jsonl"
    )
    ...
    for i in range(args.num_iterations):
        ...
        if args.val_every > 0 and val_task_datas and (i + 1) % args.val_every == 0:
            ...
            val_acc = run_inline_val(
                ...
                out_dir=os.path.join(args.experiment_dir, "val_preds"),
                run_tag=run_tag, iteration=i,
            )
            writer.add_scalar("val/math500_acc", val_acc, i)
            print(f"[VAL] iter={i}  math500_acc={val_acc:.4f}")
    ...
    ray.get(
        engines[0].collective_rpc.remote(
            ...
            args=(os.path.join(final_path, "pytorch_model.pth"),),
        )
    )
